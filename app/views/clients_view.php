      ...
      <!-- "<?php echo __HOST__; ?>/client/dashboard" -->
        <form action="<?php echo $host . "/clients/"; ?>addNewClient" id="addNewClient" class="signin-wrapper" method="post">
          ...
          <div class="form-group">
            ...
            <div class="form-group">
              <input class="form-control" placeholder="campaign" type="text" name="campaign" required>
            </div>
            <hr>
            <!--div class="form-group">
...
<div class="modal fade" id="addCampaign"  tabindex="-1" role="dialog" aria-labelledby="editCampaign">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    ...
    "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
     ...
     if(nRow.querySelector('.edit-client')){
      nRow.querySelector('.edit-client').addEventListener('click', function(event) {
        ...
        document.querySelector('#us_id_camp').innerHTML = id;
        // var s = document.getElementById('us_id_camp');
        // s.value=id;
        // console.log(id);
        $.ajax({
          type: "POST",
         ...
         if(nRow.querySelector('.addCampaign')){
      nRow.querySelector('.addCampaign').addEventListener('click', function(event) {
        var button = event.currentTarget;
        var id = button.getAttribute('attr-id');
        document.querySelector('#us_id_camp').innerHTML = id;
        // var s = document.getElementById('us_id_camp');
        ...
        $.ajax({
          type: "POST",
          url: '<?php echo $host . "/clients/"; ?>getCampaigns',
          data:  { id: id },
          success: function (data) {
            // console.log(data);
                // setTimeout(function(){
                document.querySelector('.all_campaigns').innerHTML = data;
                // }, 1000);
                    ...
                    if (!sure) {
                      ...
                      $.ajax({
                      type: "POST",
                      url: "<?php echo $host . "/clients/"; ?>deleteCampaign",
                      data: {"id": id},
                        success: function(data) {
                          document.querySelector('.ssuccess').innerHTML = '<p>' + data + '</p>';
                          setTimeout(function(){
                          $('.ssuccess').fadeOut(1000);
                          }, 3500);
         ...
         var form = $('#addCampaignForm');
           form.submit(function (ev) {
            $.ajax({
              ...
              url: '<?php echo $host . "/clients/"; ?>addCampaign',
              data:  form.serialize(),
              success: function (data) {
               console.log(data);
                document.querySelector('.ssuccess').innerHTML = '<p>' + data + '</p>';
...
//       }
    ...
    //       // var id = $('.addCampaign').attr("attr-id");
