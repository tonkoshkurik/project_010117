  ...
  function action_ajax_get()
  {
    ...
    $columns = array(
      ...
      if($d == "1") {  
        ...
        // $string .= '<a href="#" class="addCampaign" attr-id="'. $row[0] .'" attr-name="'. $row[1] .'" title="Add campaign" data-toggle="modal" class="edit-button" data-target="#addCampaign"><i class="fa fa-pencil" aria-hidden="true"></i> </a>';
    ...
    array('db'=>'`a`.`id`', 'dt'=>5, 'formatter' => function($d, $row) {
      if($d == "1") {  
        ...
        $string = '<a href="#" class="addCampaign" attr-id="'. $row[0] .'" attr-name="'. $row[1] .'" title="Added campaigns" data-toggle="modal" class="edit-button" data-target="#addCampaign"><span type="submit" class="btn btn-primary">Added campaigns</span> </a>';
...
//    echo "hi there";
  ...
  function action_editClient(){
   ...
   // var_dump($_POST);
    if($_POST["id"]){
      $id = $_POST["id"];
      $sql = ' SELECT `id`, `email`, `password`, `level`, `full_name`,  `campaign`';
      $sql.= ' FROM `users`';
        ...
        // var_dump($v);
            if($k == "id") {
              ...
              // echo '<input class="form-control" type="text" name="'.$k.'" value="'.$v.'"  > ' ;
                 echo "</div>";
                echo "<div class='form-group'>";
                echo "<label for='$k'>Campaign</label>";
                echo '<input class="form-control" type="text" name="campaign"  > ' ;
                echo "</div>";
...
//            echo $table;
  ...
  function action_getCampaigns(){
    //  $data['all'] = $this->model->getCampaigns();
    
    // $this->view->generate('dashboard_admin_view.php', 'template_view.php', $data);
    $data['all'] = $this->model->getCampaigns();
    foreach($data['all'] as $cam){
          echo "<div class='form-group'>";
          echo "<input type='hidden' name='id' value='".$cam['id']."' />";
          // echo "<label for='$k'>".$form_keys["$k"]."</label>";
          echo '<div class="form-control " >'; 
          echo $cam['campaign']; 
          echo '<a href="#" class="delete-campaign" attr-id="'. $cam['id'] .'"  title="Delete campaign"><i class="fa fa-trash-o " aria-hidden="true"></i></a></div> ' ;
        // echo ' <a href="#" class="delete-campaign" attr-id="'. $cam['id'] .'"  title="Delete campaign"><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
          echo "</div>";
    }
...
//       echo '</pre>';
       ...
       // var_dump($_POST);
          $sql = "UPDATE `users`";
          $sql.= " SET email='$email', level=$level ,full_name='$full_name', campaign='$campaign'";
          $sql.= " WHERE id='$id'";
          $con = $this->db();
          ...
          // $sql.= " SET campaign='$campaign', level=$level ,full_name='$full_name'";
          // $sql.= " WHERE id='$id'";
          // $con = $this->db();
       ...
       // var_dump($_POST);
          $sql = "UPDATE `users`";
          $sql.= " SET email='$email', password='$password', level=$level ,full_name='$full_name', campaign='$campaign'";
          $sql.= " WHERE id='$id'";
           // var_dump($sql);
          $con = $this->db();
         // var_dump($sql);
  ...
  function action_deleteCampaign(){
    ...
    $res = $this->model->deleteCampaign();
    if($res) {
      echo "Success";
      } else {
      echo "Campaign not delete! DB error";
      }
  }
  ...
  function action_addNewClient(){
    ...
    // if(isset($_POST["campaign_name"])  && isset($_POST["email"])) {
      ...
      $sql.= '(email, password, level, active, full_name, campaign )';
      $sql.= " VALUES ('$email', '$password', '$level', '$status', '$full_name', '$campaign' )";
  ...
  function action_addCampaign(){
     $res = $this->model->addCampaign();
     if($res){
      echo "Success";
    }else{
      echo "Error";
